...
#include "maybe.h"
  ...
  struct Show;
  #ifdef ARDUINO
    Str cStr(const int n) {return Str(n);}
    Str cStr(const unsigned int n) {return Str(n);}
  #else
    Str cStr(const int n) {return std::to_string(n);}
    Str cStr(const unsigned int n) {return std::to_string(n);}
  #endif

  Str cStr(const unsigned char t) {
    #ifdef ARDUINO
      return Str(t);
    #else
      return Str(1,t);
  ...
  Str cStr(const char* const t) {return t;}
  Str cStr(const I&) {return "id";}
  Str cStr(const K&o) {return "true";}
  Str cStr(const S&o) {return "S";}
  Str cStr(const B&o) {return "B";}
  Str cStr(const V&o) {return "V";}
  Str cStr(const M&o) {return "M";}
  Str cStr(const T&o) {return "T";}
  Str cStr(const Succ&o) {return "succ";}
  Str cStr(const Phi&o) {return "ϕ";}
  Str cStr(const Pred&o) {return "pred";}
  Str cStr(const True&o) {return "true";}
  Str cStr(const False&o) {return "false";}
  Str cStr(const Flip&o) {return "flip";}
  Str cStr(const Fst&o) {return "fst";}
  Str cStr(const Snd&o) {return "snd";}
  Str cStr(const Null&o) {return "null";}
  Str cStr(const _Length&o) {return "length";}
  Str cStr(const Index&o) {return "index";}
  Str cStr(const _Last&o) {return "last";}
  Str cStr(const _Concat&o) {return "concat";}
  Str cStr(const _Init&o) {return "init";}
  Str cStr(const _Reverse&o) {return "reverse";}
  Str cStr(const _Filter&o) {return "filter";}
  Str cStr(const _Map&o) {return "map";}
  Str cStr(const _FoldL&o) {return "foldl";}
  Str cStr(const _FoldR&o) {return "foldr";}
  Str cStr(const _Zip&o) {return "zip";}
  Str cStr(const _Nats&o) {return "ℕ";}
  Str cStr(const Is0&o) {return "is0";}
  Str cStr(const Add&o) {return "add";}
  Str cStr(const Nothing&o) {return "nothing";}
  Str cStr(const Just&o) {return "just";}
  Str cStr(const Text&o) {return o.data;}
  Str cStr(const Bool&o) {return o.data?"true":"false";}
  Str cStr(const Show&o) {return "show";}
  template<bool v>
  Str cStr(const StaticBool<v>&o) {return v?"true":"false";}
  template<const char* const*text>
  Str cStr(const StaticText<text>&o) {return text[0];}
  template<typename...OO>
  Str cStr(const List<OO...>&oo) {return cStr((typename List<OO...>::Base&)oo);}

  template<typename Data,Data data>
  Str cStr(const StaticData<Data,data>&o) {return cStr((Data)o);}
  template<int n>
  Str cStr(const StaticInt<n>&) {
    #ifdef ARDUINO
      return Str(n);
    #else
  ...
  // template<typename T> Str cStr(const Type<T>&o) {return showType<Type<T>>();}

  Str cStr(const Str&o) {return o;}

  template<typename T>
  Str cStr(const Data<T>&o) {return cStr(o.data);}

  template<typename Prev,typename Arg,int n>
  Str cStr(const Binder<Prev,Arg, n>&o) {return cStr(Prev())+" "+cStr(Arg());}

  template<typename Prev,typename A,typename B,int m,int n>
  Str cStr(const Binder<Prev,Binder<A,B,m>,n>&o) {return cStr(Prev())+"("+cStr(Binder<A,B,m>())+")";}

  template<typename Prev,typename Arg,int n>
  Str cStr(const Lambda<Prev,Arg, n>&o) {return cStr(o.prev)+" "+cStr(o.param);}

  template<typename Prev,typename A,typename B,int m,int n>
  Str cStr(const Lambda<Prev,Lambda<A,B,m>,n>&o) {return cStr(o.prev)+"("+cStr(o.param)+")";}

  template<typename R,typename O,typename... OO,Func<R,O,OO...>f>
  Str cStr(const Curry<Func<R,O,OO...>,f>&) {return "{c++}";}

  template<> Str cStr(const Expr<>&){return "";};
  template<typename O,typename...OO>
  Str cStr(const Expr<O,OO...>&o){return cStr(typename Expr<O,OO...>::Run());};
  ...
  // // template<int n> struct ToStr<StaticData<int,n>> {static Str type(){return cStr(n);}};
  ...
  struct Show:Combinator<Show,1> {
    ...
    template<typename O> static Text beta(const O&o) {
      return Text(cStr(std::forward<const O>(o)));
    }
