...
namespace yo {
  ...
  struct S:Combinator<S> {
    ...
    static cex auto beta(const F f,const G g,const O o)
      ->const decltype(f(forward<const O>(o))(forward<const G>(g)(forward<const O>(o))))
      {return          f(forward<const O>(o))(forward<const G>(g)(forward<const O>(o)));}
  };
  ...
  struct B:Combinator<B> {
    template<typename F,typename G,typename O>
    static cex auto beta(const F f,const G g, const O o)
      ->const decltype(f(forward<const G>(g)(forward<const O>(o))))
      {return f(forward<const G>(g)(forward<const O>(o)));}
  ...
  struct C:Combinator<C> {
    ...
    static cex auto beta(const F f,const A a, const B b)
      ->const decltype(f(forward<const B>(b))(forward<const A>(a)))
      {return f(forward<const B>(b))(forward<const A>(a));}
  };

  // λop.opp
  struct W:Combinator<W> {
    template<typename O,typename P>
    static cex auto beta(const O o,const P p)
      ->const decltype(o(forward<const P>(p))(forward<const P>(p)))
      {return o(forward<const P>(p))(forward<const P>(p));}
  };

  // λof.fo
  struct T:Combinator<T> {
    template<typename O,typename F>
    static cex auto beta(const O o,const F f)
      ->const decltype(f(forward<const O>(o)))
      {return f(forward<const O>(o));}
    static cex int /*delta*/beta(int n,int o) {return pow(n,o);}
    static double /*delta*/beta(double n,double o) {return pow(n,o);}
  };

  // λabf.fab
  struct V:Combinator<V> {
    template<typename O,typename P,typename F>
    static cex auto beta(const O o,const P p,const F f)
      ->const decltype(f(forward<const O>(o))(forward<const P>(p)))
      {return f(forward<const O>(o))(forward<const P>(p));}
  };

  // λf.ff
  struct M:Combinator<M> {
    template<typename F>
    static cex auto beta(const F f)
      ->const decltype(f(forward<const F>(f)))
      {return f(forward<const F>(f));}
  };
